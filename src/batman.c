...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/08 14:27:10 by rkirszba          #+#    #+#             */
/*   Updated: 2019/10/09 17:13:35 by rkirszba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int	batman_iteration(t_fractol *fractol, t_complex *c)
{
	int			iter;
	t_complex	z;
	...
	while (++pixel.y < limits.y1)
	{
		...
		while (++pixel.x < limits.x1)
		{
			...
			c.imag = (double)((double)pixel.y / fractol->scale\
				+ fractol->y_min);
			c = inverse_complex(&c);
			nb_iter = batman_iteration(fractol, &c);
