...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/08 14:37:06 by rkirszba          #+#    #+#             */
/*   Updated: 2019/10/09 17:55:33 by rkirszba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int	triangle_iteration(t_fractol *fractol, t_complex *c)
{
	int			iter;
	t_complex	z;
...
void		*display_triangle(void *arg)
{
	...
	while (++pixel.y < limits.y1)
	{
		...
		while (++pixel.x < limits.x1)
		{
			...
			c = cube_complex(&c);
			c = inverse_complex(&c);
			nb_iter = triangle_iteration(fractol, &c);
